...
import { mergeIncrementalADIFData, mergeIncrementalQSLDataOnly, updateRecordCount } from './adif-processor.js';
import { createBackup, restoreBackup } from './file-manager.js';
export async function performIncrementalUpdate({ username, password, localData, configContext }) {
  console.log('[LoTW-DXCC] DEBUG: Incremental update started, using config context');
  ...
  try {
    ...
    console.log('[LoTW-DXCC] Incremental:  Using timestamp:', lastUpdateTimestamp);
    console.log('[LoTW-DXCC] Incremental:  Timestamp corresponds to time:', new Date(lastUpdateTimestamp).toISOString());
    ...
    if (incrementalQsoData && incrementalQsoData.data && incrementalQsoData.data.trim()) {
      console.log('[LoTW-DXCC] Incremental:  Found new QSO data, starting merge...');
      // Update the function call around line 51
      await mergeIncrementalADIFData(incrementalQsoData.data, adifFilePath, configContext);
    } else {
    ...
    if (incrementalQslData && incrementalQslData.data && incrementalQslData.data.trim()) {
      console.log('[LoTW-DXCC] Incremental:  Found new QSL data, starting merge...');
      await mergeIncrementalQSLDataOnly(incrementalQslData.data, adifFilePath, configContext);
    ...
    // 3. 统一更新记录数量和重新生成统计
    console.log('[LoTW-DXCC] Incremental:  Recalculating and updating record count...');
    updateRecordCount(adifFilePath);
    
    console.log('[LoTW-DXCC] Incremental:  Regenerating JSON statistics based on full ADIF file...');
    ...
    if (result) {
      ...
      incrementalStats.newDXCCs = (result.dxcc_confirmed || 0) - beforeStats.dxcc;
    ...
    if (backupPath && fs.existsSync(backupPath)) {
      ...
      if (!shouldKeepBackup) {
        ...
        console.log(`[LoTW-DXCC] Incremental:  Deleting backup file: ${backupPath}`);
      } else {
        console.log(`[LoTW-DXCC] Incremental:  Keeping backup file: ${backupPath}`);
      }
    }
    ...
    console.error('[dxcc-badges: incremental-updater] Incremental update failed:', error.message);
