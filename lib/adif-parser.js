...
const schema = JSON.parse(fs.readFileSync(join(__dirname, '../schemas/lotw-dxcc-data.schema.json'), 'utf8'));
...
export function parseADIFToJSON(adifFilePath, configContext) {
  ...
  try {
    ...
    // 验证数据结构
    if (!validate(result)) {
      console.warn('[LoTW-DXCC] ADIF-parser: Data validation failed:', validate.errors);
    }
    ...
    console.log(`[LoTW-DXCC] ADIF-parser: Parsed ${stats.total_qso} QSOs, ${stats.total_qsl} QSLs, ${stats.dxcc_confirmed} confirmed DXCCs`);
...
export function extractTimestamps(header, adifData = '') {
  ...
  const qsoRxMatch = header.match(/<APP_LoTW_LASTQSORX:(\d+)>([\s\S]*?)(?=<|$)/i);
  let qsoRxValue = null;
  if (qsoRxMatch) {
  ...
  if (adifData) {
    ...
    const allQslMatches = recordsSection.match(/<APP_LoTW_RXQSL:(\d+)>([\s\S]*?)(?=<|$)/gi);
    
    if (allQslMatches) {
      for (const match of allQslMatches) {
        // 修复：移除 \s 匹配，确保能提取完整的时间戳
        const qslRecordMatch = match.match(/<APP_LoTW_RXQSL:(\d+)>([\s\S]*?)(?=<|$)/i);
        if (qslRecordMatch) {
...
function calculateDXCCStats(records) {
  ...
  for (const record of records) {
    ...
    // 修复QSL确认状态检查逻辑
    const isQslConfirmed = record.qsl_rcvd === 'Y' || 
  ...
  // 计算已确认的DXCC实体数量
  const dxcc_confirmed = Object.values(dxccStats).filter(stats => stats.qsl > 0).length;
...
function convertToTimestamp(lotwTimestamp) {
  try {
    ...
    console.warn(`[LoTW-DXCC] ADIF-parser: Failed to convert timestamp: ${lotwTimestamp}`);
    return 0;
  }
}
