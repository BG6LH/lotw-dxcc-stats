 ...
 */
async function requestWithRetry(url, config, maxRetries = 3, baseDelay = 5000, progress = null) {
  let lastError;
  
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      if (progress && attempt > 1) {
        progress.updateMessage(`Retrying request (attempt ${attempt}/${maxRetries})`);
      }
      ...
      // 对于503错误或超时错误，进行重试
      if ((status === 503 || error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT') && attempt < maxRetries) {
        // 指数退避策略：每次重试延迟时间翻倍
        const delayTime = baseDelay * Math.pow(2, attempt - 1);
        
        console.log(`[LoTW-DXCC] LoTW request failed (attempt ${attempt}/${maxRetries}): ${error.message}`);
        console.log(`[LoTW-DXCC] Waiting ${delayTime/1000} seconds before retry...`);
        
        if (progress) {
          progress.updateMessage(`Request failed, waiting ${delayTime/1000}s before retry`);
        }
 ...
 */
async function _fetchDataFromLoTW({ username, password, queryParams, configContext, progressMessage, onDownloadProgress }) {
  ...
  try {
    ...
    // 使用重试机制的请求
    const response = await requestWithRetry(configContext.get('lotwUrl'), {
    ...
    const finalSize = ProgressIndicator.formatFileSize(response.data?.length || 0);
 ...
 */
export async function fetchADIFData({ username, password, qsoBeginDate: paramqsoBeginDate, configContext }) {
  // 优先使用传入的参数，其次使用配置
  const qsoRxSince = paramqsoBeginDate !== undefined ? paramqsoBeginDate : configContext.get('qsoBeginDate');
  
  console.log('[LoTW-DXCC] Lotw-update: Starting to fetch Full ADIF data from LoTW...');
 ...
 * 获取增量 QSL 数据
 */
export async function fetchIncrementalQSL({ username, password, sinceTimestamp, configContext }) {
  const { timestampToLoTWQueryFormat } = await import('./timestamp-utils.js');
  const qslSince = timestampToLoTWQueryFormat(sinceTimestamp);
  console.log(`[LoTW-DXCC] Lotw-update: Fetching incremental QSL data since: ${qslSince}`);
 ...
 * 获取增量 QSO 数据
 */
export async function fetchIncrementalQSO({ username, password, sinceTimestamp, configContext }) {
  const { timestampToLoTWQueryFormat } = await import('./timestamp-utils.js');
  const qsoSince = timestampToLoTWQueryFormat(sinceTimestamp);
  console.log(`[LoTW-DXCC] Lotw-update: Fetching incremental QSO data since: ${qsoSince}`);
