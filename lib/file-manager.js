...
export async function createBackup(filePath, configContext) {
  ...
  if (!shouldBackup) {
    console.log(
      `[LoTW-DXCC] File-manager: Backup disabled, skipping backup: ${filePath}`,
    );
    return null;
  }
  ...
  if (!fs.existsSync(filePath)) {
    console.log(
      `[LoTW-DXCC] File-manager: File does not exist, skipping backup: ${filePath}`,
    );
    return null;
  }
...
function cleanupOldBackups(filePath, configContext) {
  try {
    ...
    if (backupFiles.length > 1) {
      ...
      filesToDelete.forEach((file) => {
        try {
          ...
          console.log(
            `[LoTW-DXCC] File-manager: Deleting old backup: ${file.path}`,
          );
        } catch (error) {
          console.warn(
            `[LoTW-DXCC] File-manager: Failed to delete old backup: ${file.path}, ${error.message}`,
          );
        }
      });
    }
    ...
    console.warn(
      `[LoTW-DXCC] File-manager: Error occurred while cleaning old backups: ${error.message}`,
    );
  }
}
...
export function loadLocalData(jsonFilePath) {
  if (fs.existsSync(jsonFilePath)) {
    try {
      ...
      console.warn(
        `[LoTW-DXCC] Lotw-update: Failed to read local JSON data: ${error.message}`,
      );
      return null;
    }
  }
...
export function restoreBackup(backupPath, originalPath) {
  ...
  try {
    ...
    console.log(
      `[LoTW-DXCC] File-manager: Restored backup from: ${backupPath} to: ${originalPath}`,
    );
